...
[[nodiscard]] bool has_ply_extension(std::string_view path) {
  constexpr std::string_view extension = ".ply";
...
[[nodiscard]] Result<std::string> validate_path(std::string_view path) {
  ...
  if (!has_ply_extension(path)) {
    return Result<std::string>::failure(
        path_error(ErrorCode::unsupported_format, "extension must be .ply", path));
  }
...
Result<OwnedSurface> read_ply(std::string_view path_utf8, LengthUnit unit, FrameId frame) noexcept {
  auto path = validate_path(path_utf8);
  if (!path) {
    return Result<OwnedSurface>::failure(std::move(path).error());
  }
  return backends::pcl_backend::read_ply_file(path.value(), unit, std::move(frame));
}

Result<std::size_t> write_ply(std::string_view path_utf8, SurfaceView surface,
                              PlyEncoding encoding) noexcept {
  auto path = validate_path(path_utf8);
  if (!path) {
    return Result<std::size_t>::failure(std::move(path).error());
  }
  return backends::pcl_backend::write_ply_file(path.value(), surface,
